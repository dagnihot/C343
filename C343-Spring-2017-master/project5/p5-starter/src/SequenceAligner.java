...
public class SequenceAligner {
  ...
  private void fillCache() {
	  int gap = judge.getGapCost();

	    cache[0][0] = new Result(0, Direction.NONE);
	    for(int i = 1; i < m+1; i++)
	        cache[0][i] = new Result(i*gap, Direction.LEFT);

	    for(int i = 1; i < n+1; i++)
	        cache[i][0] = new Result(i*gap, Direction.UP);

	    for(int i = 1; i < n+1; i++){
	        for(int j = 1; j < m+1; j++){
	            // Set to M
	            int score = judge.score(x.charAt(i - 1), y.charAt(j - 1));
	            Result res = new Result(cache[i-1][j-1].getScore() + score, Direction.DIAGONAL);
	          
	            // Iff I would be greater than M, set to I
	            if(cache[i][j-1].getScore() + gap > res.getScore())
	                res = new Result(cache[i][j-1].getScore() + gap, Direction.LEFT);
	          
	            // Iff D would be greater than current score, set to D
	            if(cache[i-1][j].getScore() + gap > res.getScore())
	                res = new Result(cache[i-1][j].getScore() + gap, Direction.UP);
  ...
  private void traceback() {
   ...
   Result result = cache[n][m];
   Direction parent = result.getParent();
   result.markPath();
   while (row >= 1 || col >= 1){
	   if (parent == Direction.UP){
		   alignedX = x.charAt(row - 1) + alignedX;
	   ...
	   if (parent == Direction.LEFT){
		   ...
		   alignedY = y.charAt(col - 1) + alignedY;
		   
		   col--;
	   }
	   
	   if (parent == Direction.DIAGONAL){
		   alignedX = x.charAt(row - 1) + alignedX;
		   alignedY = y.charAt(col - 1) + alignedY;
